...
<div class="content shop_line_pag"  >
    ...
    <div class="shop_line" >
     <div class="shop_line_box" >
             <h2>订单号：<span>${myorder.id} </span></h2>
             <p>收货人：<span>${user.username}</span></p>
             <p>收货地址：<span>${user.address}</span></p>
             <p>总金额：<span>${totalPrice}</span></p>
             <p>时间：<span><fmt:formatDate value="${myorder.createtime}" pattern="yyyy-MM-dd HH:mm:ss"/></span></p>
            <p>订单状态：<span>${myorder.isorder}</span></p>
              <a href="orderinfo.action"><p>查看详情</p></a>
        </div>
